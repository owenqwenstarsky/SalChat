...
export function BrandIcon({ icon, size = 42, background }: { icon: IconSpec; size?: number; background?: string }) {
  ...
  return (
    <View style={[styles.wrap, { width: size, height: size, borderRadius: size * 0.28, backgroundColor: fill }]}>
      {brand ? <Svg width={size * 0.54} height={size * 0.54} viewBox="0 0 24 24"><Path d={brand.path} fill={theme.text} /></Svg> : icon.value === 'openai' ? <OpenAiMark size={size * 0.58} color={theme.text} /> : <Text style={{ fontSize: size * 0.42, fontWeight: '700', color: theme.text }}>S</Text>}
    </View>
  );
}
