...
import { Divider, Field, GhostButton, PrimaryButton, Section } from '@/components/UI';
...
import { providerKindName } from '../(tabs)/models';
...
export default function ProviderDetailScreen() {
  ...
  return (
    <Screen title={provider.displayName} eyebrow={providerKindName(provider.kind)} action={<Pressable onPress={() => router.back()}><Ionicons name="close" size={28} color={theme.text} /></Pressable>}>
      <View style={[styles.endpoint, { backgroundColor: theme.surface, borderColor: theme.line }]}><BrandIcon icon={provider.icon} size={48} /><View style={{ flex: 1 }}><Text numberOfLines={1} style={[styles.url, { color: theme.text }]}>{provider.baseUrl}</Text><Text style={[styles.meta, { color: theme.muted }]}>{credentials.length} accounts · {models.length} models</Text></View></View>
      ...
      <Section title="Models" description="Discovery is optional. Any wire model ID can be configured manually.">
        <View style={[styles.group, { backgroundColor: theme.surface, borderColor: theme.line }]}>{models.map((model, index) => <View key={model.id}>{index ? <Divider /> : null}<Pressable style={styles.row} onPress={() => router.push({ pathname: '/model/[id]', params: { id: model.id } })}><BrandIcon icon={model.icon} size={38} /><View style={{ flex: 1 }}><Text style={[styles.name, { color: theme.text }]}>{model.displayName}</Text><Text style={[styles.meta, { color: theme.muted }]}>{model.wireId}</Text></View><Ionicons name="chevron-forward" size={18} color={theme.muted} /></Pressable></View>)}</View>
      ...
      <GhostButton danger onPress={() => Alert.alert('Delete provider?', 'Models and credentials will be removed. Existing chat messages keep their provenance labels.', [{ text: 'Cancel', style: 'cancel' }, { text: 'Delete', style: 'destructive', onPress: () => void deleteProvider(provider.id).then(() => router.replace('/(tabs)/models')) }])}>Delete provider</GhostButton>
    </Screen>
  );
}

const styles = StyleSheet.create({
  endpoint: { borderWidth: 1, borderRadius: radius.lg, padding: 16, flexDirection: 'row', gap: 13, alignItems: 'center', marginBottom: 12 }, url: { fontWeight: '700', fontSize: 14 }, meta: { fontSize: 12, marginTop: 3 },
  actions: { flexDirection: 'row', justifyContent: 'flex-end', gap: 8, marginBottom: 34 }, group: { borderWidth: 1, borderRadius: radius.lg, overflow: 'hidden' }, row: { minHeight: 64, paddingHorizontal: space.lg, flexDirection: 'row', alignItems: 'center', gap: 12 },
  avatar: { width: 38, height: 38, borderRadius: 12, textAlign: 'center', textAlignVertical: 'center', lineHeight: 38, fontWeight: '800' }, name: { fontSize: 15, fontWeight: '700' }, current: { fontSize: 9, letterSpacing: 1.1, fontWeight: '900' }, form: { marginTop: 18 },
  iconChoices: { flexDirection: 'row', flexWrap: 'wrap', gap: 9 }, iconChoice: { width: 48, height: 48, borderRadius: 15, borderWidth: 2, alignItems: 'center', justifyContent: 'center' }, random: { fontSize: 21 }, twoCol: { flexDirection: 'row', gap: 10 },
});
